...
function calculateStudyDebt() {
    ...
    if (getActualStudyHours() === getIdealStudyHours()) {
        ...
        return `Tebrikler Toplam ${getActualStudyHours()} saat çalışarak hedefi tutturdunuz`

    } else if (getActualStudyHours() > getIdealStudyHours()) {

        return `Mukemmel Toplam ${getActualStudyHours()} saat çalıştınız. Bu hafta ${getActualStudyHours() - getIdealStudyHours()} saat fazla çalıştınız`

    } else {

        return `Malesef Toplamda ${getActualStudyHours()} saat çalıştınız  hedefinizden  ${getIdealStudyHours() - getActualStudyHours()} saat az çalıştınız`
    }
}
